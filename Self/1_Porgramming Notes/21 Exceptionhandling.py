...
'''try block and except block are complement to each other , i.e. only one block can run at a time'''
#? try.....except block
...
'''
When try block executed then only else block will executed but if instead of try block ,except block
will execute then else block will be skipped .
'''
x= int(input("enter 1st number : "))
y= int(input("enter 2nd number : "))
